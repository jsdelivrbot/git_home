...
lasso_fit <- function(lasso_matrix){
    ...
    train_set <- cbind(lasso_matrix[, -c(1, 163)], interact_terms)
    
    lasso_model <- cv.glmnet(y=lasso_matrix[, "price"],
                             x=train_set,
                            nfolds=10,
                            alpha=1
                            )
    ...
    RMSE_in <- mean((predict(lasso_model, train_set) - training_set_4lasso[, "price"])^2)^.5
...
lasso_prediction_RMSE <- function(lasso_model, lasso_set){
    ...
    test_set <- cbind(lasso_matrix[, -c(1, 163)], interact_terms)
    RMSE <- mean((predict(lasso_model, test_set) - lasso_matrix[, "price"])^2)^.5
...
lasso_prediction <- function(lasso_model, lasso_set, raw_data){
    ...
    predict_set <- cbind(lasso_matrix[, -c(1, 163)],interact_terms)
    
    raw_data[, "price"] <- predict(lasso_model, predict_set)
...
out_data[, "pice"] <- lasso_prediction(lasso_model, prediction_set_4lasso, out_data)

write.csv(out_data, "Cars_X_out_lasso.csv")
```
