...
The barplot shows the projection of PC1 on each topics, which is equivalent to the weight of each topic contained in PC1. From the projection pattern it is observed `food`, `parenting`, `religion`, `sports_fandom` and `school` are the most represented in PC1. Repeating the above process across PC2 to PC18, more combinations can be found, and thereby generating information of such combinations, the company could differentiate its messages from others and attract specific consumer groups.s